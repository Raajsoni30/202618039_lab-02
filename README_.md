...
The objective of this assignment is to practice vectorized programming with NumPy and basic data wrangling with Pandas using the Titanic dataset.
...
* Calculate the minimum, maximum, median, mean, and standard deviation.
...
* Compare the sample statistics with the selected mean and standard deviation.
...
Answer the following questions using Boolean indexing and/or `query()`:

1. Count male passengers older than 50.
2. Find the number of female first-class passengers and their survival percentage.
3. Find passengers aged 20–40 with a Fare above the overall median who survived.
4. Find passengers travelling alone, aged below 30, who did not survive.
5. Find passengers who embarked from Southampton, were in Pclass 2 or 3, and paid more than the Southampton median Fare.

## Task 6: GroupBy and Aggregation

* Calculate the survival rate by Sex.
* Calculate the survival rate by Pclass.
* Calculate the average Age and Fare by Pclass.
* Calculate passenger count and survival rate by Sex and Pclass.
...
* Calculate Q1, Q3, IQR, and the 1.5 × IQR bounds for Fare.
...
* Write 5–7 observations based on the numerical results and visualizations.
...
The Titanic dataset is used to apply these techniques to a real-world dataset and understand the relationships between passenger characteristics and survival.
